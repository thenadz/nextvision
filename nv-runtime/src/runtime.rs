...
mod tests {
    ...
    /// No-op stage for testing pipeline plumbing.
    struct NoOpStage;
    ...
    /// Stage that panics on every process call.
    struct PanicStage;
    ...
    fn multi_feed_registration() {
        ...
        let h1 = runtime
            .add_feed(build_config(vec![Box::new(NoOpStage)], Box::new(s1)))
            .unwrap();
        let h2 = runtime
            .add_feed(build_config(vec![Box::new(NoOpStage)], Box::new(s2)))
            .unwrap();
        let h3 = runtime
            .add_feed(build_config(vec![Box::new(NoOpStage)], Box::new(s3)))
            .unwrap();
    ...
    fn start_stop_lifecycle() {
        ...
        let handle = runtime
            .add_feed(build_config(vec![Box::new(NoOpStage)], Box::new(sink)))
            .unwrap();
    ...
    fn backpressure_drops_are_reported() {
        ...
        // Check that the feed completed.
        let m = handle.metrics();
        // With 20 frames pushed rapidly into a depth-1 queue, some drops expected.
        // Note: exact drop count depends on timing; just verify the mechanism works.
        // frames_received should be 20, frames_dropped + frames_processed ~= 20.
        assert!(
            m.frames_received > 0,
    ...
    fn restart_window_reset_still_works_with_nonzero_max() {
        ...
        // This is the same as the existing `restart_window_resets_counter`
        // test but explicitly confirms nonzero max behavior is unaffected
        // by the max_restarts=0 guard.
        let runtime = Runtime::builder()
